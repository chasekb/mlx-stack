...
- Updated `ai_dev/core/stack_ops.py` compose command resolution to prefer direct `podman-compose` when present, avoiding macOS `podman compose` fallback to Docker Compose contexts/sockets; removed the obsolete top-level compose `version` key from the canonical compose file.
- Updated default and checked-in LiteLLM model mappings so public aliases like `local-mlx` forward to host MLX artifact paths like `models/local-mlx`, preventing alias names from being treated as Hugging Face repository ids by `mlx_lm.server`.
